...
<img src="/assets/transformerstream.png" style="width: 50%; display: block; margin: 0 auto;" />
...
A critical, often overlooked, aspect is that transformers do not simply predict "the most likely next token." They are optimized to model entire **probability distributions** over all possible continuations. The cross-entropy loss

$$\mathcal{L} = -\sum_{t \in V} p_{\text{true}}(t) \log p_{\text{model}}(t)$$

rewards the network for accurately emitting the entire distribution.
...
When the true underlying distribution is sharply peaked (e.g., in predicting the next token in a piece of code), these factors may align early with a single token vector, leading to an "early collapse" pattern observable via the logit lens. Conversely, when the distribution is broad (e.g., in creative prose), the factors can remain abstract until the final few layers, where multiple token vectors are softly superposed. This distributional perspective underpins the transformer's abilities in in-context learning and generalization, allowing adaptation to novel patterns without explicit parameter updates. Crucially, this adaptability stems from the transformer's stateless autoregressive process: the generation of each token is grounded by a re-evaluation of the complete discrete symbolic history. This constant re-grounding enables the self-attention mechanism to dynamically shape the internal measure over continuation worlds from the input prefix, effectively reconfiguring latent factors to condition the output distribution. Such contextual modulation of the predictive measure—achieved without the continuity biases inherent in explicitly stateful recurrent models—allows a single pretrained transformer to exhibit highly adaptive, specialized behaviors based on few-shot prompting. The resulting richness of this context-dependent learned measure also provides a robust basis for diverse sampling strategies (e.g., nucleus sampling, temperature scaling) that modulate the output distribution to control generative stochasticity and creativity.
