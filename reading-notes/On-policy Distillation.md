...
A crucial detail is that we do not backprop through the sampling distribution $p_{s} (\cdot|x)$. That means the process is broken into two distinct steps 1) generate sequences using the student model and 2) calculate loss on fixed sequence y, and the gradient is only calculated in the second step.
### GKD and RL
